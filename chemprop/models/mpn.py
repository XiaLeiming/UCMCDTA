...
class MPNEncoder(nn.Module):
    ...
    def forward(self,mol_graph: BatchMolGraph, features_batch=None) -> torch.FloatTensor:
        f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope, bonds, a2a, b2b  = mol_graph.get_components()
        self.args.cuda = not self.args.no_cuda and torch.cuda.is_available()
        if self.args.cuda or next(self.parameters()).is_cuda:

            f_atoms, f_bonds, a2b, b2a, b2revb, a2a, b2b= (
                    f_atoms.cuda(), f_bonds.cuda(), 
                    a2b.cuda(), b2a.cuda(), b2revb.cuda(), a2a.cuda(), b2b.cuda())
        ...
        for depth in range(self.depth - 1):
            ...
            # 聚合点，
            lei_agg_atom_message = index_select_ND(message_atom, a2a)
            lei_agg_atom_message = lei_agg_atom_message.sum(dim=1) * lei_agg_atom_message.max(dim=1)[0]
            lei_agg_bond_message = index_select_ND(message_bond, b2b)
            lei_agg_bond_message = lei_agg_bond_message.sum(dim=1) * lei_agg_bond_message.max(dim=1)[0]
            message_atom = message_atom + lei_agg_atom_message  + agg_message
            message_bond = lei_agg_bond_message
...
class MPN(nn.Module):
    ...
    def forward(self, batch: Union[List[str], BatchMolGraph],
        ...
        if not self.graph_input:  # if features only, batch won't even be used
            batch = mol2graph(batch, self.args)         #     return BatchMolGraph(mol_graphs, args)
